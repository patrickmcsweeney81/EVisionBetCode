...
2025-12 updates:
- Raw odds logger (`core/raw_odds_logger.py`) now writes **one row per outcome** with all bookmaker columns populated, and headline `book`/`price` restricted to AU books. Fair odds are centralized in `core/fair_odds.py`.
- Added `nba_outliers.py` helper to scan `data/raw_odds.csv` for AU-book outliers versus sharp median; see Usage below.
...
INCLUDE_US_BOOKS=0               # 1 to include mass‑market US books in ACTIVE_BOOKIES
...
- **EV_MIN_EDGE**: Edge computed as `(fair_prob * book_odds - 1)`; only edges ≥ threshold logged when filter enabled.
...
## Output Files
- `data/raw_odds.csv`: Comprehensive raw+fair odds snapshot (one row per outcome with all books).
- `data/all_odds_analysis.csv`: Legacy combined snapshot (deprecated; prefer `raw_odds.csv`).
...
### NBA Outlier Scanner
Lightweight helper to surface AU-book mispricings against sharp median:
```powershell
python nba_outliers.py --csv data/raw_odds.csv --threshold 0.05 --min-sharps 2 --limit 30
```
Adjust `threshold` and `min-sharps` to control noise (props often need higher values).
