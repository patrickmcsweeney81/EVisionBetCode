...
from .utils import devig_two_way, snap_to_half
...
def process_nfl_props_event(event: Dict, prop_markets: List[str] = None) -> List[Dict]:
    """
        ...
        for player_name, lines in players.items():
            for line in lines:
                ...
                for bk in bookmakers:
                    ...
                    if bkey == "pinnacle":
                        pinnacle_odds = extract_nfl_prop_odds(
                            bk, player_name, market_key, line, include_au=False
                        )

                    if bkey in SHARP_BOOKIES:
                        odds = extract_nfl_prop_odds(
                            bk, player_name, market_key, line, include_au=False
                        )
                        if odds:
                ...
                for bk in bookmakers:
                    ...
                    if bkey not in SHARP_BOOKIES:
                        continue
                    odds_pair = extract_nfl_prop_odds(
                        bk, player_name, market_key, line, include_au=False
                    )
    ...
    for market_key, players in player_markets.items():
        for player_name in players:
            ...
            for bk in bookmakers:
                ...
                if bkey == "pinnacle":
                    pinnacle_odds = extract_nfl_prop_odds(
                        bk, player_name, market_key, line=None, include_au=False
                    )
            ...
            for bk in bookmakers:
                bkey = bk.get("key", "")
                odds = extract_nfl_prop_odds(
                    bk, player_name, market_key, line=None, include_au=True
                )
                if odds:
                    all_book_odds[bkey] = odds
