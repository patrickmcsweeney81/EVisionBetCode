...
from .utils import devig_two_way, effective_back_odds_betfair, snap_to_half
...
def extract_totals_odds(
    bookmaker_data: Dict, line: float, betfair_commission: float = 0.06, include_au: bool = False
) -> Dict[str, float]:
    """
...
def process_totals_event(
    event: Dict, target_line: float, betfair_commission: float = 0.06, sport: Optional[str] = None
) -> Dict:
    """
    ...
    if bf_over and bf_under:
        ...
        if bf_over_prob > 0 and bf_under_prob > 0:
            bf_key = (
                "betfair_ex_au"
                if "betfair_ex_au" in [bk.get("key") for bk in bookmakers]
                else "betfair_ex_uk"
            )
    ...
    fair_prices_v2 = build_fair_prices_two_way(
        bookmaker_odds_over, bookmaker_odds_under, market_type="main", sport=sport
    )
    ...
    return {
        ...
        "betfair": {"over": bf_over, "under": bf_under},
