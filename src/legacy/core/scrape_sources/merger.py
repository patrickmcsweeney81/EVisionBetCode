...
def find_matching_event(
    scraped_event: ScrapeEvent, primary_events: List[Dict], time_tolerance_minutes: int = 30
) -> Optional[Dict]:
    """
    ...
    for primary in primary_events:
        ...
        try:
            primary_time = datetime.fromisoformat(
                primary.get("commence_time", "").replace("Z", "+00:00")
            )
            scraped_time = datetime.fromisoformat(
                scraped_event.commence_time.replace("Z", "+00:00")
            )
...
def merge_bookmaker_into_event(primary_event: Dict, scraped_event: ScrapeEvent) -> Dict:
    """
...
def merge_odds_data(
    primary_events: List[Dict], scraped_events: List[ScrapeEvent], time_tolerance_minutes: int = 30
) -> List[Dict]:
    """
