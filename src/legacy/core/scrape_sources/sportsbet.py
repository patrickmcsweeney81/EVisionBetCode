...
class SportsbetAdapter(BookmakerAdapter):
    ...
    def _fetch_competition_events(
        self, sport_key: str, sport_path: str, comp_id: str, markets: Optional[List[str]]
    ) -> List[Event]:
    ...
    def _parse_market(self, raw: Dict, requested_markets: Optional[List[str]]) -> Optional[Market]:
        ...
        for sel in selections:
            ...
            outcomes.append(Outcome(name=name, price=price, point=point))

        if not outcomes:
            return None

        return Market(key=market_key, outcomes=outcomes, last_update=raw.get("lastUpdate"))
