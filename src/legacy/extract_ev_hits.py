...
def extract_ev_hits():
    ...
    ev_hits = df[
        (df["ev_numeric"] >= EV_MIN_THRESHOLD) & (df["ev_numeric"] <= EV_MAX_THRESHOLD)
    ].copy()
    ...
    print(f"\nResults:")
    print(
        f"  Extreme EVs (>{EV_MAX_THRESHOLD}%):      {extreme_count:,} rows (excluded - data quality)"
    )
    print(f"  Negative EV (<0%):    {negative_count:,} rows (unfavorable)")
    print(
        f"  Small positive (<{EV_MIN_THRESHOLD}%):  {total_rows - ev_hits_count - extreme_count - negative_count:,} rows (low edge)"
    )
    print(
        f"  ✓ EV HITS ({EV_MIN_THRESHOLD}%-{EV_MAX_THRESHOLD}%):   {ev_hits_count:,} rows -> {OUTPUT_CSV}"
    )
    print(f"\nTop 10 EV hits:")
    for idx, row in ev_hits.head(10).iterrows():
        stake = row["stake"] if row["stake"] else "$0"
        print(
            f"  {row['ev']:>8} | {row['selection'][:30]:30} | {row['book']:15} | Stake: {stake:>6}"
        )
    print(f"{'='*70}\n")
