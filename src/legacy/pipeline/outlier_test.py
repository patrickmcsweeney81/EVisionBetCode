...
AU_BOOKS = [
    'Sportsbet', 'Pointsbet', 'Tab', 'Tabtouch', 'Betr', 'Neds', 'Boombet'
]

OUTLIER_THRESHOLD = 0.03  # 3%

META_COLS = {
    "timestamp", "sport", "event_id", "away_team", "home_team", 
    "commence_time", "market", "point", "selection"
}
...
def find_outliers():
    ...
    if outliers:
        # Build headers
        headers = [
            'highlight', 'sport', 'event_id', 'event', 'market', 'selection', 'point',
            'au_book', 'au_odds', 'max_sharp', 'edge_percent'
        ] + bookmaker_cols
        ...
        print("\nTop 10 Outliers:")
        print("-" * 70)
        for i, opp in enumerate(sorted(outliers, key=lambda x: x['edge_percent'], reverse=True)[:10], 1):
            print(f"{i:2}. {opp['selection']:30} | {opp['au_book']:12} {opp['au_odds']:.2f} vs Sharp {opp['max_sharp']:.2f} → +{opp['edge_percent']:.2f}%")
    else:
