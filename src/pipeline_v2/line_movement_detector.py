...
class LineMovementDetector:
    ...
    def __init__(
        self, previous_odds_csv: Optional[Path] = None, current_odds_csv: Optional[Path] = None
    ):
        """
    ...
    def detect_movements(self) -> List[PriceMovement]:
        ...
        for current_key, current_row in self.current_odds.items():
            ...
            try:
                ...
                # Calculate movement
                movement = self._calculate_movement(previous_row, current_row, old_odds, new_odds)
    ...
    def _calculate_movement(
        prev_row: Dict, curr_row: Dict, old_odds: float, new_odds: float
        ...
        return PriceMovement(
            ...
            new_odds=new_odds,
            old_extracted_at=(
                datetime.fromisoformat(prev_row.get("timestamp", ""))
                if prev_row.get("timestamp")
                else None
            ),
            new_extracted_at=(
                datetime.fromisoformat(curr_row.get("timestamp", ""))
                if curr_row.get("timestamp")
                else None
            ),
...
class PropAlertDetector:
    ...
    def detect_from_ev_opportunities(self, ev_opportunities: List[Dict]) -> List[PropAlert]:
        ...
        for opp in ev_opportunities:
            ...
            best_side = (
                "OVER" if "Over" in selection else "UNDER" if "Under" in selection else "UNKNOWN"
            )
