"""
NFL Fair Odds Calculation

Specialization for NFL:
- Conservative outlier removal (3%) due to weekly events
- Minimum 1 sharp required (fewer books available)
- Weight sharps: Pinnacle 60%, DraftKings 40%
"""
...
class NFLFairOdds:
    """NFL-specific fair odds calculation"""
    
    SPORT_NAME = "NFL"
    OUTLIER_THRESHOLD = 0.03  # Conservative
    MIN_SHARP_COUNT = 1
    
    # NFL weight profile (hidden from users)
    WEIGHT_PROFILE = {
        "pinnacle": 0.60,
        "draftkings": 0.40,
    }
    ...
    def calculate_fair_odds(self, market_data: Dict) -> Tuple[float, float, str]:
        ...
        notes = f"NFL | O:{len(over_odds)} U:{len(under_odds)} | {over_notes} | {under_notes}"
    ...
    def _calculate_side_fair(self, odds_list: List[Dict], side_name: str) -> Tuple[float, str]:
        ...
        # Remove outliers (conservative: 3%)
        ...
        # Calculate weighted average using NFL weight profile
